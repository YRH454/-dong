...
.stat .item{flex:1;background:var(--w);border-radius:12px;padding:14px;text-align:center;box-shadow:0 1px 4px rgba(0,0,0,.04)}
...
.fg input,.fg select,.fg textarea{width:100%;padding:0 14px;border:1.5px solid #e0e0e0;border-radius:10px;font-size:16px;outline:none;font-family:inherit;transition:border .15s}
.fg input,.fg select{height:46px}
.fg textarea{height:90px;padding:12px 14px;resize:vertical}
.fg input:focus,.fg select:focus,.fg textarea:focus{border-color:var(--bl);box-shadow:0 0 0 3px rgba(30,136,229,.08)}
...
if(tm!=null){
  ...
  tm1.setTm(tm); tm1.setBz(request.getParameter("bz"));
  for(int i=0;i<n;i++){tm1.setTm(tm); tmDao.add(tm1,adminid);}
  pageContext.setAttribute("msg","添加成功，跳转中...");
  response.setHeader("refresh","2;url=plchuti.jsp");
}
%>
<div class="stat">
  <div class="item"><div class="num"><%=shangxian%></div><div class="lbl">上限</div></div>
  <div class="item"><div class="num"><%=yxsl%></div><div class="lbl">已出</div></div>
  <div class="item"><div class="num"><%=shangxian-yxsl%></div><div class="lbl">剩余</div></div>
</div>
<div class="form-card">
<form id="form1" action="" method="post">
  <div class="fg"><label>出题数量</label>
  <select name="tmgs">
  <% for(int i=shangxian-yxsl;i>=1;i--){ out.print("<option value="+i+">"+i+" 题</option>"); } %>
  </select></div>
  ...
  <input type="button" value="批量出题" class="btn" onclick="chuti()">
</form>
