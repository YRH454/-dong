...
<van-pull-refresh v-model="refreshing" @refresh="load" class="dash-page">
  ...
  <div class="stat-hero">
    ...
    <div v-if="isRoot" class="stat-hero-card" style="border-left-color:#C62828">
      <b style="color:#C62828">{{ deptCount }}</b>
      <span>管理部门</span>
    </div>
  </div>
  ...
  <!-- Recent Activity -->
  <div class="section-card">
    <h4 class="section-title">最近操作记录</h4>
    <div class="log-list">
      <div class="log-item" v-for="(l,i) in logs" :key="i">
        <span class="log-dot"></span>
        <span class="log-text">{{ l.content }}</span>
        <span class="log-time">{{ l.createTime }}</span>
      </div>
      <div v-if="!logs.length" class="empty-inline">暂无记录</div>
    </div>
  </div>
...
<script setup>
import { ref, reactive, onMounted, onUnmounted, onActivated, nextTick } from 'vue'; import { showToast } from 'vant'; import api from '../../api'
import * as echarts from 'echarts'

const pieChart = ref(null)
const refreshing=ref(false),zt=ref(0),teacherStats=ref([]),exporting=ref(false),logs=ref([]),deptCount=ref(0),isRoot=ref(false)
...
function renderChart(data) {
  ...
  chart.setOption({
    ...
    series: [{
      ...
      data: [
        { value: data.selectedCount||0, name: '已选题', itemStyle: { color: '#4CAF50' } },
        { value: unselected, name: '待选题', itemStyle: { color: '#e0e0e0' } },
        { value: data.studentCount-(data.selectedCount||0), name: '未选学生', itemStyle: { color: '#FF9800' } }
      ]
    }]
  })
}

async function load(){try{const r=await api.get('/admin/stats');cards[0].value=r.studentCount;cards[1].value=r.teacherCount;cards[2].value=r.topicCount;cards[3].value=r.selectedCount;zt.value=r.zt;teacherStats.value=r.teacherStats||[];isRoot.value=r.isRoot||false;deptCount.value=r.deptCount||0;await nextTick();renderChart(r)}catch(e){};try{const l=await api.get('/admin/op-logs');if(l.code===0)logs.value=l.list||[]}catch(e){};refreshing.value=false}
...
.chart-card { background: #fff; border-radius: 14px; padding: 16px; box-shadow: 0 2px 12px rgba(0,0,0,.04) }
.chart-card h4 { font-size: 15px; font-weight: 700; color: #333; margin-bottom: 8px }
...
.log-dot { width: 6px; height: 6px; border-radius: 50%; background: #FB8C00; margin-top: 6px; flex-shrink: 0 }
.log-text { flex: 1; color: #555; line-height: 1.5 }
.log-time { font-size: 11px; color: #bbb; white-space: nowrap }
.empty-inline { text-align: center; padding: 16px; color: #ccc; font-size: 14px }
